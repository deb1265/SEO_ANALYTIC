...
const ResultsSection: React.FC<ResultsSectionProps> = ({ 
  ...
  const tabs: { id: TabType; label: string; icon: string; isNew?: boolean }[] = [
    ...
    { id: 'aiinsights', label: 'Evidence & Insights', icon: 'fa-brain' },
  ...
  const generateTextReport = (data: AnalysisData): string => {
    ...
    return `
SEO HTML CHECKLIST REPORT
================================
URL: ${data.url}
Date: ${data.analyzedAt}
Source: ${data.source || 'unknown'}
Mode: ${data.analysisMode || 'rules'}
...
Content structure: ${ai.scores?.keywords?.score || 0}/25
Technical SEO: ${ai.scores?.technical?.score || 0}/30
HTML accessibility hints: ${ai.scores?.uxMobile?.score || 0}/20
...
Generated by SEO Analyzer Pro
  ...
  return (
    <section className="results-section fade-in">
      ...
      <div className="analysis-banner">
        <div className="banner-content">
          <div className="banner-icon">
            <i aria-hidden="true" className="fas fa-robot"></i>
          </div>
          <div className="banner-text">
            <h3>HTML audit complete</h3>
            <p>15 evidence-based checks · {data.source === 'import' ? 'Imported HTML' : 'Fetched HTML'} · {data.analysisMode === 'rules+ai' ? `Optional AI: ${aiModel}` : 'No AI required'}</p>
          </div>
        </div>
      ...
      {(data.warnings || []).map((warning, i) => <p role="status" key={i}>{warning}</p>)}
      ...
      <div className="action-buttons">
        ...
        <button onClick={() => downloadReport('txt')} className="action-btn btn-red">
          <i aria-hidden="true" className="fas fa-file-pdf"></i> Download Report
        </button>
        <button onClick={() => downloadReport('json')} className="action-btn btn-blue">
          <i aria-hidden="true" className="fas fa-code"></i> Export JSON
        </button>
        <button onClick={onReset} className="action-btn btn-gray">
          <i aria-hidden="true" className="fas fa-redo"></i> New Analysis
        </button>
      </div>
    </section>
  );

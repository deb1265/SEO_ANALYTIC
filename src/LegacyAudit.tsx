...
function LegacyAudit() {
  ...
  const [apiSettings, setApiSettings] =
    useState<ApiSettings>(buildInitialSettings);
  ...
  const updateLoadingStep = useCallback(
    (progress: number, status: string, currentStep: string) => {
  ...
  const saveApiSettings = (settings: ApiSettings) => {
    ...
    if (typeof window !== "undefined") {
      try {
        window.localStorage.setItem(
          SETTINGS_STORAGE_KEY,
          JSON.stringify({ aiModel: settings.aiModel }),
        );
      } catch {
  ...
  const handleAnalyze = async (
    ...
    try {
      const html =
        htmlInput !== undefined
          ? htmlInput
          : await fetchPageContent(url, allowProxies);
      ...
      if (useAI && apiSettings.openRouterKey) {
        try {
          ...
          warnings.push(
            "AI suggestions were unavailable or invalid. The free HTML audit is complete.",
          );
        }
      }
      if (
        ...
        try {
          keywords = await fetchKeywordSuggestions(
          ...
          warnings.push(
            "Keyword provider unavailable. No search-volume data was collected.",
          );
        }
      }
      ...
      setAnalysisData({
        ...
        analysisMode: mode,
        source: htmlInput !== undefined ? "import" : "network",
        warnings,

...
export async function handleApi(req: Request, env: any) {
  ...
  try {
    ...
    if (u.pathname.startsWith("/api/reports/") && req.method === "PATCH") {
      ...
      if (
        input.state &&
        ["draft", "reviewed", "delivered"].includes(input.state)
      )
      ...
      if (input.ranking) {
        ...
        if (
          ...
          k.position < 1 ||
          k.position > 1000 ||
          !["Search Console", "SERP tool", "Manual observation"].includes(
            k.source,
          )
        )
          return json(
        ...
        if (report.rankings.length >= 1000)
          return json(
            { error: "This report has reached its observation limit." },
            400,
          );
      ...
      report.revision = row.revision + 1;
